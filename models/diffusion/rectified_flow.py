    ...
    def training_losses(
        ...
        if dynamics_enhance:
            ...
            if vis_root_path:
                ...
                aux_loss_ = F.normalize(aux_loss_clone[:, :4].reshape(bs, -1), p=1).reshape(
                    bs, 4, aux_loss_c, aux_loss_h, aux_loss_w
                )
                ...
                for map_id in range(4):
                    fig, ax = plt.subplots()
                    ax.matshow(
                        loss_map[map_id], cmap="viridis", interpolation="lanczos", vmin=0, vmax=1
                    )
                    ...
                    fig, ax = plt.subplots()
                    ax.matshow(
                        loss_map[map_id], cmap="hot", interpolation="lanczos", vmin=0, vmax=1
                    )
            ...
            aux_w = 1 + torch.cat(
                (torch.zeros(aux_w.shape[0], 1, *aux_w.shape[2:]).to(aux_w), aux_w), dim=1
            )
