        ...
        for i, (img, rot_matrix) in tqdm(enumerate(val_data)):
            ...
            for t1 in range(0, args.test_video_frames, args.temporal_patch_size):
                ...
                for t2 in range(args.temporal_patch_size):
                    ...
                    pose = torch.cat(
                        (pose[:, 1:condition_frames, ...], predict_pose, predict_pose), dim=1
                    )
                    yaw = torch.cat(
                        (yaw[:, 1:condition_frames, ...], predict_yaw, predict_yaw), dim=1
                    )
                ...
                for t2, img_pred in enumerate(img_preds):
                    img_pred_np = (img_pred.permute(1, 2, 0).numpy() * 255).astype("uint8")[
                        :, :, ::-1
                    ]
                    ...
                    cv2.imwrite(
                        os.path.join(video_save_path, "%d.png" % (t1 + t2 + condition_frames)),
                        img_pred_np,
                    )
