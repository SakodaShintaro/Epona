...
def train(local_rank, args):
    ...
    model = TrainTransformersDiT(
        args, local_rank=local_rank, condition_frames=args.condition_frames // args.block_size
    )
    ...
    eff_batch_size = (
        args.batch_size * args.condition_frames // args.block_size * dist.get_world_size()
    )
    ...
    train_data = DataLoader(train_dataset, num_workers=32, batch_sampler=mix_data_sampler)
    ...
    while step < args.iter:
        ...
        for i, (img, rot_matrix) in enumerate(train_data):
            ...
            if step % 100 == 1 and rank == 0:
                ...
                writer.add_scalar(
                    "loss/loss_yaw_pose", loss_final["loss_yaw_pose"].to(torch.float32), step
                )
                writer.flush()
            if rank == 0:
                logger.info(
